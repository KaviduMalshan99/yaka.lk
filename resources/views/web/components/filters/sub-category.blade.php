<div class="col-md-6 col-lg-12">
    <div class="product-widget">
        <h6 class="product-widget-title">{{GoogleTranslate::trans('sub categories', app()->getLocale())}}</h6>
        <ul class="product-widget-list product-widget-scroll">
            @foreach($subCategories as $subcategory)
                <li class="product-widget-item">
                    <a href="{{route('ads',[$subcategory->url])}}">
                        <span class="product-widget-text">{{GoogleTranslate::trans($subcategory->name, app()->getLocale())}}</span>
                        @php
