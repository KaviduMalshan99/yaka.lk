            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="single-content">
                        <h2>{{$nowSearchSubCategoryUrl==0 ? GoogleTranslate::trans($category->name, app()->getLocale()) : GoogleTranslate::trans(preg_replace('/-/', ' ', $nowSearchSubCategoryUrl), app()->getLocale())}}</h2>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{route('/')}}">{{GoogleTranslate::trans('Home', app()->getLocale())}}</a></li>
                            <li class="breadcrumb-item active" aria-current="page">{{$nowSearchSubCategoryUrl==0 ?  GoogleTranslate::trans($category->name, app()->getLocale()): GoogleTranslate::trans(preg_replace('/-/', ' ', $nowSearchSubCategoryUrl), app()->getLocale())}}</li>
                        </ol>
                    </div>
                </div>
            </div>
                        ...
                        <div class="col-lg-12">
                            <div class="header-filter">
                                <div class="filter-show">
                                    <span style="text-transform: capitalize"><i class="fas fa-map-marker-alt" style="margin-right: 10px;"></i>  @if($nowSearchLocation == 0) {{GoogleTranslate::trans('All Locations', app()->getLocale())}} @else {{ GoogleTranslate::trans(preg_replace('/-/', ' ', $nowSearchLocation), app()->getLocale()) }}@endif</span>
                                </div>
                                <div class="filter-short">
                                    <span style="text-transform: capitalize"><i class="fas fa-tags" style="margin-right: 10px;"></i> {{$nowSearchSubCategoryUrl==0 ? GoogleTranslate::trans($category->name, app()->getLocale()) : GoogleTranslate::trans(preg_replace('/-/', ' ', $nowSearchSubCategoryUrl), app()->getLocale()) }}</span>
                                </div>
